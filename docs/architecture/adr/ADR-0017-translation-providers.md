...
3. **Licence-honouring cache policy, per provider.** Bundled = compiled-in (permanent). LicensedApi = **in-memory, session-bounded cache only where the licence permits; NO on-disk persistence of licensed text by default** (API.Bible does not grant offline/caching by default). The cache is bounded (LRU by chapter, fixed capacity) per the no-unbounded-growth rule. UserSupplied = stored like a bundle, but flagged with the user's own responsibility notice in the register/UI.
...
- **Negative:** licensed translations are **online-only by default** (a genuine product limitation to set owner expectations on); a new "fetching" reply state must be added additively (skip-if-none, fixtures preserved); quota/report failures need surfacing in the console's status line.
