...
- Status: **Accepted** — **revision 4**, ratified 2026-09-20 by the Software Architect (Aria) in a dedicated session, independently of the ticket that drafted it. See "Revision 4 — Software Architect ratification" at the end of this document for what was re-derived and what was accepted as a named limitation. Revisions 2 and 3 remain Accepted on their own terms. Revision 1 recommended a design that the S1 spike falsified and revision 2 replaced its central mechanism. Revision 3 re-opened nothing: D1–D4 stood unchanged, and it extended D5 only, to cover the second virtualizer 86akgqdxr adds to the same file, introducing no new mechanism and no new WRITE CLASS. **Revision 4 is different in kind, and says so plainly: it DOES add a new D5 exemption class** (`jump`) for 86akgqdw0's click-triggered "jump to a transcript timestamp" feature — the first genuinely new class since D5 was written. D1–D3 stand unchanged; D4's "what converged" is unaffected; only D5's exemption vocabulary grows, by exactly one class, exactly one site.
- Date: 2026-09-14 (rev 2) · 2026-09-20 (rev 3) · 2026-09-20 (rev 4)
- Confidence: **High** for revision 2 — unlike revision 1, its central mechanism is measured rather than reasoned, on both engines, with negative controls, by two independent sessions; the residual unknowns named in "Before this is accepted" are about the *real component*, not the mechanism. **High** for revision 3 on a deliberately narrower basis: it introduces no new mechanism and no new class of `scrollTop` write, and the existing safety argument was re-checked against the second container's actual call graph rather than assumed to carry over (see "Revision 3"). **Medium** for revision 4, stated honestly: the reasoning is sound and leans on premises this ADR already measured (C1/C2, and the already-accepted "the user asked to go somewhere else" trade-off), but — unlike revisions 2 and 3 — **no new spike or mutation battery was run for this specific write**; it is a reasoned argument from existing evidence, not new measurement. Explicitly flagged for the four-reviewer gate's Cody and Sana to confirm or reject before this ships (see "Revision 4" below and this ticket's Goal Contract). **The architect's ratification leaves this at Medium deliberately** — Medium describes the *evidentiary basis* (reasoned, not freshly measured), and that description stays true after review. It is not a reservation about the conclusion: the one fact a fresh spike could add is the cancellation revision 4 already concedes against itself, so measuring it could not change the decision. Confidence and acceptance are answering different questions here, and conflating them would misreport both.
- Owner: Software Architect, with Frontend Engineer (delivery), QA and Performance Engineer. **Revision 4 was drafted inline by 86akgqdw0's implementing engineer** (Backend Engineer role), mirroring revision 3's own precedent of being written by the ticket that needed it rather than through a separate architect dispatch — recorded here explicitly because, unlike revision 3, this one adds a genuinely new decision, not just an extension of an existing one, so the provenance matters more. **That provenance is now closed:** the drafting engineer did not move this to `Accepted`; a Software Architect session that wrote none of the code did, after re-deriving the argument from the ADR text, the shipped code and its controls (see the ratification section at the end).
- Relates: ADR-0002 / ADR-0003 (the WebView is the operator console only), FR-130, NFR-019, AC3 of 86akcffvt; 86akgqdxr (rev 3); 86akgqdw0/FR-124 (rev 4)
...
> `transcripts.js` contains **no assignment to `.scrollTop`**, except sites in these three classes, each of which must carry a marker comment naming **its own class**:
>
...
> - **`D5-exempt(jump)`** (rev 4) — a deliberate, click-triggered, one-shot navigation write: the user asked to go to a specific point in the transcript, and nothing else can put them there in a virtualized list where the target row may not be mounted. See "Revision 4" below for the full argument.
>
> There is no fourth class. Any write reachable from a `scroll`, `wheel`, `keydown` or animation-frame handler falls in none of the three and is forbidden outright — that is precisely the write C1 says cannot be made correct.
...
- **`jump` — 1** (rev 4): `jumpToOffsetMs`'s single `scrollTop` write, the transcript log's landing-position write for a clicked note-item timestamp (86akgqdw0). See "Revision 4" below.
...
This is **measured, not argued.** A seven-case mutation battery was run against the real `scripts/operator_headless.py` over isolated copies of `dist/` (positive control green, six mutations red, 7/7). Its M3 case deletes the detections panel's `init` reset and adds a reactive `detLogEl.scrollTop` write inside `onDetScroll` — the precise defect class this ADR exists to prevent — leaving five marked sites. The naive flat-count-of-5 check **passes M3**; the per-class check fails it on both classes. The other cases cover an unmarked reactive write, one silenced with a copied class marker, a bare classless `D5-exempt`, an unknown class, and a *deleted* exemption (so the numbers are guarded downward as well as upward). **Rev 4 re-ran the narrow slice of this that applies to a NEW class** rather than the full seven-case battery: add a second, unmarked `jump`-class-shaped write and confirm the per-class check goes RED on `jump` specifically (not just "some" count mismatch), then revert. This is smaller than the original battery because rev 4 adds no new mechanism for the OTHER classes to interact with — it is one more independent class in an already-proven per-class counting scheme, not a new scheme.

Per-class counts also scale: a further virtualizer or write class raises the relevant count by however many sites it needs, each recorded here in its own revision. As before, **a new exemption cannot be added by marking it; only a revision of this ADR can grow any number.**
...
## Revision 4 — a genuinely new exemption class: clicking a note item's timestamp jumps to it (86akgqdw0)

86akgqdw0 (FR-124) adds timestamps to generated sermon-note items (chapter markers, and where a confident match exists, outline points) and asks that clicking one "jumps to/highlights that point in the Transcripts viewer" — the transcript log this ADR already governs.

**Why this cannot be done without a `scrollTop` write, and therefore cannot be done without touching D5.** The log is virtualized: at most `WINDOW_ROWS` (150) real rows are ever mounted, and a jump target arbitrarily far into a long transcript will usually not be one of them. There is no DOM node to `scrollIntoView()` and no native browser primitive that scrolls a container to an *unmounted* logical position — that is the entire reason virtualization needs its own scroll math in the first place. Setting the scroll position of the mounted window to the target's computed offset is therefore not an implementation choice this revision is picking over an alternative; it is the only mechanism that exists for "jump to an arbitrary index in a virtualized list", exactly as D2's own "Costs and risks" section already anticipated for a big scrollbar-drag jump ("a big jump with no surviving anchor node... the user asked to go somewhere else").

**This is not an extension of an existing class.** `init` is a reset that runs once, before any transcript is even visible, with nothing to conflict with. `test-hook` exists solely because the headless driver cannot dispatch trusted input. A jump is neither: it is real, user-triggered, mid-session interaction with content already on screen. Revision 3 could truthfully say it "introduces no new mechanism and no new class of `scrollTop` write" (see its own text above); revision 4 cannot say that, and does not try to.

### D5 (extended) — a third class: `jump`

> **`D5-exempt(jump)`** — a deliberate, click-triggered, one-shot navigation write, issued from a `click` event handler and reachable from nowhere else. Exactly **one** site: `jumpToOffsetMs`'s `logEl.scrollTop = ...` assignment, which computes the target row's exact Fenwick-tree offset (`offsetAt`, the same function `window.__trOffsetAt` already exposes), mounts the window around it via the existing `renderWindow` (a pure DOM mutation, not a `scrollTop` write — D2's own machinery, unmodified), and only then sets `scrollTop` to land there.

**Why a `click` handler is safe under C1/C2 in a way `scroll`/`wheel`/`keydown`/an animation frame is not.** C1's hazard is a script write racing an *in-flight native scroll animation* the browser is currently driving — which is why D5 forbids exactly the four handler types (`scroll`, `wheel`, `keydown`, animation-frame) that can fire *while* such an animation is in progress and are the mechanism by which the old, deleted provenance-classification code tried to referee that race. A `click` on a timestamp badge is not part of that race: it is not itself a scroll gesture, it does not fire during a scroll/wheel/keydown-driven animation as a matter of *routine* use (the two are different input channels — pointer vs. keyboard/wheel), and when it fires there is nothing algorithmically special about the moment that would make cancelling an in-flight animation *wrong* rather than merely coincidental.

**The one honest overlap, named rather than hidden.** If a reader is genuinely mid-keyboard-scroll (holding Page Down, say) and clicks a timestamp badge at that exact instant, the click's `scrollTop` write DOES cancel whatever native animation was in flight, per C1. This ADR does not claim otherwise. It claims the outcome is *correct*, not merely tolerable: the click is an explicit request to be somewhere else in the transcript, and finishing the interrupted keyboard scroll first would be the surprising behaviour, not this one. This is the identical judgement D2's "Costs and risks" section already made and shipped for a scrollbar-drag jump with no surviving anchor — revision 4 applies the same standard to a second, narrower kind of deliberate jump rather than inventing a new one.

**What was and was not re-verified.** Unlike revisions 2 and 3, no new spike was run against real WebKit/Chromium for this specific write — the honest reason this revision's Confidence is stated as Medium, not High. What WAS done: a targeted mutation check confirming the per-class D5 static check actually distinguishes this class from the other two (add a second, unmarked `jump`-shaped write; confirm `scripts/operator_headless.py`'s D5 check goes RED specifically citing the unmarked line; revert). What was NOT done: a fresh engine-level measurement of click-during-keyboard-animation on real WebKit. The argument for safety rests on C1/C2's EXISTING measurements (a click is not one of the four named reactive handlers) plus the ADR's own already-accepted reasoning for a deliberate jump, not on new data.

**This is why revision 4 is explicitly flagged for the four-reviewer gate, not merely landed.** 86akgqdw0's Goal Contract names Cody and Sana specifically to confirm or reject: (a) whether reasoning-from-existing-evidence is sufficient for a genuinely new write class, versus requiring a fresh spike the way revisions 2/3's *mechanism* changes did; and (b) whether an inline revision by the implementing ticket — the same process revision 3 used — remains appropriate for a change that, unlike revision 3's, adds a new decision rather than extending an existing one. If either reviewer disagrees, the fallback (recorded in that Goal Contract) is to ship 86akgqdw0's timestamp/copy-export functionality without the click-to-jump interaction, and revisit the jump behaviour under a dedicated Software-Architect-led revision.

**Alternative considered and rejected: no jump at all, timestamp display only.** Would need no ADR change. **Rejected as leaving real, ticket-required value on the table** rather than a safety concern — the acceptance criterion explicitly asks for the jump — but recorded as the genuine, always-available fallback if revision 4's reasoning does not survive review (see above).

**Alternative considered and rejected: `scrollIntoView()` on the target row.** Only works when the target row is already mounted (at most 150 of them are, at any position). A general "jump to any index" feature would still need the `scrollTop` fallback for the common case of an unmounted target, so this would not remove the need for a `jump` exemption — it would only add a second code path with its own D5 question. Rejected for the complexity, not the safety.

**Consequences.** The exemption budget grows from two classes to three (`init` 2, `test-hook` 3, `jump` 1) — every `D5-exempt` site must still name its own class, and the per-class check now recognises `jump` as valid rather than treating it as an unknown-class failure. `docs/architecture/adr/ADR-0026-...md` and `scripts/operator_headless.py`'s `D5_EXPECTED_EXEMPT_COUNTS`/comments/error strings are updated together, in the same commit, so the documented invariant and the enforced one cannot drift apart even briefly.

**Four-reviewer gate on this revision (PR #51).** Cody and Sana were asked explicitly to confirm or reject the two questions in "This is why revision 4 is explicitly flagged" above.

- **Cody:** confirmed the write-site safety argument independently (exactly one `scrollTop` write outside `init`/`test-hook`, reachable only from the one click handler). Pushed back on the inline-authorship process specifically because this revision's own text (above) admits it adds a new decision, not merely an extension of one — asked for a dedicated Software Architect's sign-off before this leaves `Proposed`.
- **Sana:** independently confirmed the same structural claim (`jumpToOffsetMs` has exactly one caller, the click listener) and agreed the safety argument holds — a fresh WebKit/Chromium spike would only re-measure a fact revision 4 already concedes (the "one honest overlap" above). Her condition matches Cody's: an implementing engineer may draft a new decision but should not be the authority moving it to `Accepted`; a Software Architect follow-up should ratify that transition. Status therefore stays **Proposed** pending that follow-up, not merely as this document's default but as the reviewers' explicit condition.
- Both reviewers also found the SAME real defect independently (Cody traced it first): `link_timestamps`'s positional fallback (`selahcue-core/src/providers.rs`) could place an unmatched chapter marker's fallback offset chronologically BEFORE an earlier, already-matched marker's real offset — a Rust-side bug, not a flaw in this ADR's own write-site argument, but one that would have made the `jump` feature land on a genuinely wrong target for a mixed matched/unmatched marker list. Fixed by interpolating each unmatched run against its nearest already-resolved neighbours rather than by ordinal position among only the other unmatched markers; regression-tested and mutation-verified (RED against the reverted bug, GREEN restored).
- Sana additionally found the existing headless jump checks (both classes) only ever exercised an ALREADY-mounted target (a 3-segment fixture, well under `WINDOW_ROWS`) — the trivial case, not the one this exemption exists for. Closed by extending the timestamps fixture with 300 filler segments so one marker's target is genuinely unmounted before the click, asserted explicitly (`__trRowFor(...) === null` beforehand, mounted+highlighted after).
- Sana's control-gap note (D5 counts write SITES, not who may CALL `jumpToOffsetMs`) is closed by a companion static check, `check_jump_call_site_is_click_only` in `scripts/operator_headless.py`: `jumpToOffsetMs` may be called from exactly one site, and that site's own line must register for `"click"` and must not sit alongside `scroll`/`wheel`/`keydown`/`requestAnimationFrame`. Mutation-verified against two isolated mutated copies (the call moved to a `"scroll"` registration; a second call site added elsewhere) — both RED, with the same file restored after.

**Unchanged.** D1–D4 in full. The `init`/`test-hook` classes, their counts, and every revision-2/3 consequence and verification-bar item. This revision makes no claim about the transcript log's height metric, anchoring, or any existing write — it adds one new, narrowly-scoped write and the reasoning for it.

---

## Revision 4 — Software Architect ratification (2026-09-20)

**Status change: `Proposed` → `Accepted`.** Reviewed by the Software Architect (Aria) in a dedicated session at commit `868c23f` (PR #51 head, base `main` @ `4b21c39`), from an isolated worktree pinned to that SHA. This is the sign-off Cody and Sana each made a condition of the transition, and it is deliberately **not** a countersignature of their reviews: the argument was re-derived from the ADR text, the shipped code and the controls, and would have been rejected on its own terms had it not held.

**Why a separate session was the right condition to impose.** Revision 4's own text concedes it adds a new decision rather than extending one. An author is a poor judge of whether their own new decision is narrow enough, because the scoping choice and the justification for it come from the same reasoning — this is the same separation-of-duties principle that already stops any role approving its own security posture. Revision 3's inline precedent does not transfer, and the drafting engineer was right to flag that rather than lean on it.

**What was independently verified** (not taken from the two reviews):

- **The write sites.** Six `scrollTop` assignments in `transcripts.js`, each marked: `init` at lines 831/875, `test-hook` at 617/622/778, `jump` at 670. Matches `D5_EXPECTED_EXEMPT_COUNTS` exactly. No unmarked write.
- **The caller.** `jumpToOffsetMs` is defined once (line 658) and called once (line 1107), inside `makeTimestampBtn`'s `"click"` registration on a `<button type="button">`. There is no programmatic `.click()` anywhere in the file, so the handler is reachable only from genuine user activation.
- **The controls actually bite.** An eight-case mutation battery of the architect's own construction, run against the real `scripts/operator_headless.py` over isolated copies: unmarked write, second write carrying a copied `jump` marker, call moved into a `scroll` handler, second call site, call deferred into `requestAnimationFrame`, call registered for `keydown`, and two refactor-shaped probes — **8/8 RED, positive control GREEN.**

**One point the revision's text understates, examined and found not to be a hole.** A `<button>` synthesises a `click` on keyboard activation, so "this is a click handler, therefore not a keyboard path" is not quite true as written. It does not matter here, for two independent reasons: a focused button consumes Space/Enter as activation rather than scrolling its ancestor, so the activating keypress cannot itself be driving an animation; and the badge lives in the draft/notes list (`li` children), a different scroll container from `#tr-detail-log`, which is the element written. The residual case — keyboard-scrolling the log, then clicking a badge with the pointer — is exactly the "one honest overlap" the revision already names and accepts.

**The trade-off is accepted on the same terms the ADR argues.** A deliberate navigation request should win over an in-flight scroll the user has already abandoned by making it. Finishing the interrupted keyboard scroll first would be the surprising outcome. This is D2's existing, shipped judgement for a scrollbar-drag jump applied to a narrower case, not a new one.

**A fresh spike was considered and is not required.** It could only measure whether a click-triggered write cancels an in-flight keyboard animation — and C1 already establishes that *any* write does, including a no-op. Revision 4 concedes that cancellation against itself and argues the outcome is correct. A measurement cannot adjudicate a design judgement, so the spike would return a known answer to a question the decision does not turn on. Conceding the worst case is stronger evidence here than measuring it would be.

**Accepted with one named limitation — the call-site check is textual and line-local.** `check_jump_call_site_is_click_only` inspects the single call site's own **line**. Two consequences, both confirmed by running it:

- It is **conservative on refactors**: moving the call onto its own line inside a legitimate multi-line `"click"` registration turns the check RED. That is a false positive, but it fails safe and is cheap to resolve.
- It is **evadable by a deliberately misleading line**: a call site whose line carries a quoted `"click"` inside a comment while the real registration is a `scroll` handler on the next line passes GREEN. Verified live.

This is accepted rather than treated as blocking, for the same reason this repository already accepts the `RELEASE_UNSAFE_FEATURES` regex's limits: a text check is a tripwire against drift, not a defence against an author working to defeat it, and every *accidental* path tested goes red. The difference worth recording is that the Makefile case is backstopped by an executable probe testing the guard's effect, and this one has no behavioural equivalent. **Do not describe this check as constraining "who may call `jumpToOffsetMs`"** — it constrains what that call's own line says. A future revision that needs a stronger guarantee should add a behavioural control (assert the jump does not fire during a synthetic scroll), not extend the regex.

**Not re-opened.** D1–D4, the `init`/`test-hook` classes and counts, and every revision-2/3 consequence. This ratification adds no decision; it accepts one.
