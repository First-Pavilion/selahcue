...
**Amendment (86ak69u8u, PR #96 security review — Sana).** The `DeviceToken.token_hash` item this entry "tracked separately" was closed on the same reasoning recorded above: high-entropy CSPRNG input (`_generate_full_token`, 160 bits), a genuinely write-only column (`authenticate_device_token` never reads it, verified by tracing the call site), and no user/wire-supplied value can ever reach it — the exact conditions under which a keyed fast hash is strictly stronger at rest than PBKDF2. `token_hash` now stores `HMAC-SHA256(SECRET_KEY, "device-token-hash:" + token)` in place of `make_password(token)`. This amendment records that closure; it did not itself go through a fresh product-owner review the way DEC-013's own mint change did — the review round (Sana, security; Quinn, QA) is the record of unanimous technical agreement, ADR-0022 is amended to match, and `CustomerSession.token_hash` (the one other column DEC-013's own condition 3 named as unchanged) remains on `make_password` by that same, still-standing, condition.
