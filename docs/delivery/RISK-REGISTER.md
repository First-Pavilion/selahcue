...
| RISK-008 | TTS audio routing — accidental playback through main sound system, or audio feedback into transcription mic | Safety/UX | 3 | 4 | 12 | Explicit output-device selection; default to operator/preview routing; clear active-TTS status; audit in Stage 4 + Stage 11; feedback-safety tests | AI + QA | Open |
...
| RISK-011 | Not under version control — no git repo, so work is not yet tracked/rollback-safe | Delivery | 3 | 3 | 9 | Propose `git init` + baseline commit as first Stage 1 follow-up (pending user approval — repo currently non-git) | Delivery | Open |
