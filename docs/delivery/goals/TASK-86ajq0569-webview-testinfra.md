...
- Iter 2 (C-003 wiring): `.github/workflows/ci.yml` — after the Chrome step, an `Install Playwright WebKit (Linux)` step + a `Webview WebKit-engine smoke (Linux)` step (`SELAHCUE_WEBKIT_REQUIRE=1`). Commit `da53774` → **3-OS CI `30686139914` GREEN**; the operator-Linux log shows the WebKit smoke ran on the runner: `=== WebKit smoke: 5 checks, 0 FAIL ===` (Playwright installed + `--with-deps webkit` worked + the smoke passed on real WebKit). Both harness logs green (Chrome 66/0 + WebKit 5/0). Result: PASS.
- Iter 3 (C-004 review + CI hardening): adversarial review `wf_9095cf04-439` (bite-falsepass + ci-wiring; the first run stubbed) — bite-falsepass **INFO×2** (both gates hard-bounded + fail-closed, no false-pass); ci-wiring raised **1 MEDIUM + 2 LOW**, all CI-durability (each "fails loud, never a false green" — confirmed by the green run). **All fixed:** (MEDIUM) `pip install --user` could refuse on a PEP-668 externally-managed image → added `--break-system-packages`; (LOW) unpinned playwright → pinned `playwright==1.60.0` so the WebKit engine is reproducible; (LOW) the operator job had no `timeout-minutes` → added `timeout-minutes: 25` so a wedged install fails fast. YAML re-validates. Result: PASS pending the hardening re-CI (verified by conclusion). See `CODE-REVIEW-batch-webview-testinfra.md`.
