...
| **`cargo doc`** | Run by NEITHER `make ci` nor any CI job, so a doc link naming a function that no longer exists is invisible to every gate. This branch renamed two functions and removed one; three stale links resulted and none was caught by a gate — one was found only by running `cargo doc` by hand | Medium, and it outlives this PR |
...
- **No gate runs `cargo doc`.** `make ci` does not, and no CI job does, so broken intra-doc links — including links to functions that have been renamed or deleted — reach `main` unnoticed. This branch renamed two functions and removed one; three stale links resulted and no gate caught any of them. **Fits `86ak5rjh7`**, which CLAUDE.md already uses for exactly this class ("`selahcue-stt` is linted by nothing", the local-gate coverage gaps) — fit checked against the ticket's stated scope in CLAUDE.md rather than assumed, and that ticket is real and pre-existing, not invented here. Measured baseline for whoever picks it up: `origin/main` carries **10 unresolved intra-doc links** across core/lan/app, so adding the gate means triaging those first.
