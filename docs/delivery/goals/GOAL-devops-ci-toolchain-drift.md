...
| C-017 | yes | `--no-fail-fast` measurably surfaces results it would otherwise hide | (a) SYNTHETIC: inject a failing test into two independent crates, run with and without; (b) REAL: count suites at or after the failing binary on CI run 32896017248 | (a) 1 binary FAILED without, 2 with — **injected, not a real failure**; (b) 51 of 138 suites never report without the flag | recorded terminal output; GitHub run 32896017248 | PASS |
