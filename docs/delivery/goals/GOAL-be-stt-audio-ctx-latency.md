...
- FINDING (materially diverges from the ticket, escalated per this contract's own pause condition, not silently absorbed): at `audio_ctx = 512`, windows of 6-9 s reproduce Vera's reported improvement cleanly (independently measured on this machine: ~360-460 ms, vs. a ~1.03-1.15 s reproduced pre-fix baseline). A window of EXACTLY 10 s — `engine.rs`'s `max_utterance_samples` default, the longest a real utterance ever gets before force-close — does NOT: independently measured at ~1.4-1.5 s, essentially a wash against the pre-fix baseline rather than the ticket's stated 450-490 ms. An ad-hoc sweep found 700-800 measures clean at exactly 10 s, but this PR does NOT adopt it: the same sweep found `audio_ctx = 650` triggers a hard native crash (`GGML_ASSERT(nb01 % 8 == 0)` in whisper.cpp's Metal backend `ggml-metal.m`, SIGABRT) — evidence that this parameter space has unsafe values close to plausible-looking ones, so a value this session alone tuned is not shipped without Vera's validation. This PR ships the ticket's own validated 512 and documents the 10 s edge case as a passing regression-ceiling test (not the optimistic target) rather than overclaiming or silently dropping the discrepancy. Owner: performance-engineer (Vera) — explicitly flagged for her review pass, not blocking this PR's draft/review cycle.
...
| C-007 | yes | Perf re-measurement: before/after decode-time figures reproduced on this machine (not just quoted from Vera's report) for at least a 6 s and a 10 s window | Manual timing harness run against the real model, both pre-fix (baseline `FullParams`, no cap) and post-fix (capped) | Post-fix materially faster than pre-fix at 6-9 s windows (reproduced: ~1.03-1.15s -> ~360-460ms); the 10 s window is a documented DIVERGENCE from Vera's 450-490ms figure (reproduced instead: ~1.4-1.5s, a wash vs. baseline) — reproduced-with-a-flagged-exception, not a clean match | before/after numbers + the 10s-boundary finding recorded in PR description + ClickUp evidence comment; divergence escalated to Vera per this contract's own pause condition, not silently resolved — see Assumptions/Risks | PASS |
...
- Risks: (1) a genuinely short final utterance may see the same quality risk Vera measured at 2 s interim windows — flagged as a residual assumption for Vera to confirm at review, not blocking given the urgent lag defect. (2) `single_segment(true)` changes `close_utterance`'s behaviour for an utterance whisper.cpp would otherwise have split into multiple internal segments (previously emitted as multiple duplicate-timestamped `RecognizedSegment`s) into one merged segment — read as a strict improvement (removes a pre-existing timestamp-duplication artifact) but called out for reviewer awareness.
