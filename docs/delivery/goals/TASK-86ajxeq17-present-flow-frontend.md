...
| C-006 | yes | a11y: `role=grid` + roving tabindex; three separable rings (inset selection / outset focus / live+label); aria-live announces live changes; `--sc-*` tokens only | headless + `grep -nE "var\(--(live|panel|line|accent)\)"` on new rules → none | PASS | headless + grep | PENDING |
