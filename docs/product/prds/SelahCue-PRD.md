...
| FR-059 | Configurable "TIME UP" state at zero (text, colour, background, flash/static, per-output) | MVP | At 0 the configured TIME UP state renders on selected outputs only; wording customisable; **default is solid-inverted with a calm ≤0.5 Hz pulse on the TIME UP words only** (measured within FR-175: worst 0.500 red flashes/sec against the 3.0 limit, pulsing area 4.18 % against WCAG's 25 % small-safe limit — see ARCH-UX-REVIEW-stage5 M4); any further flashing is likewise constrained by FR-175 | brief §timer; FLOW-003 |
...
| FR-075 | Crash recovery + crash-loop breaker | MVP | After forced shutdown, relaunch restores live content, plan position, and running timers (countdowns re-anchored to wall-clock, count-up resumes accumulated). **Crash-loop breaker:** after **3 crashes within 60s** the app does not silently auto-resume — it offers "Resume last live state" vs "Start clean / skip last item" and disables the suspected offending item | FLOW-008; MAJOR-02 |
...
| FR-159 | Linux secret-storage fallback never silently plaintext | MVP | With no Secret Service present, secrets are encrypted in a local vault with an Argon2id passphrase-derived key — never plaintext (OD-19 resolved, ARCHITECTURE §11). MVP: mobile-pairing tokens are MVP secrets and need custody from day one | threat §4; OD-19 |
...
| FR-175 | Seizure-safety / reduced-motion | MVP | Flashing output is bounded to ≤3 **general** flashes/sec **and** ≤3 **red** flashes/sec **and** within the small-safe flashing-area limit (WCAG 2.3.1) — verified by the ADR-0015 flash-rate/luminance/red-transition analyzer, not a bare frequency count; default TIME UP is solid-inverted with a measured ≤0.5 Hz word pulse (FR-059); a reduced-motion option disables non-essential animation | MAJOR-12; M4; WCAG 2.3.1 |
...
| NFR-021 | Screen-reader support (desktop) | MVP | MVP: core live desktop controls (next/prev/clear/blackout/go-live, timer/TIME-UP, scripture-stage) expose accessible names/roles to platform screen readers, **and the current live state (on-air content/next/timer) is announced via an aria-live region fed by a structured live-scene text payload** (ADR-0015 §13); full screen-reader coverage of all UI is R2 | a11y; MAJOR-13; M3 |
...
**R3 (transcription):** FR-099, FR-100, FR-101, FR-102, FR-103, FR-104, FR-105, FR-106, FR-107, FR-108, FR-109, FR-110, FR-131, FR-132, FR-133, FR-134, FR-135, FR-136, FR-137, FR-152, FR-153, FR-154, FR-156, FR-158, FR-166, FR-167, FR-170, FR-171, FR-172, FR-177
