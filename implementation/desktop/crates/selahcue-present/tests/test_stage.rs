...
fn timer_bar_colour_reflects_state() {
    ...
    // OK (green), full fill.
    let ok = render(&compose_stage(None, None, &running(1.0), &theme, 200, 100));
    ...
    let fb = render(&compose_stage(None, None, &warn, &theme, 200, 100));
    ...
    let fb = render(&compose_stage(None, None, &up, &theme, 200, 100));
...
fn current_and_next_regions_show_text_when_present() {
    ...
    let fb = render(&compose_stage(Some(&current), Some(&next), &running(1.0), &theme, 320, 240));
...
fn empty_state_shows_no_text() {
    let theme = StageTheme::dark();
    let fb = render(&compose_stage(None, None, &running(1.0), &theme, 320, 240));
    assert!(!has_color(&fb, theme.text), "with no content there is no slide text");
}
...
fn main_and_stage_show_different_scenes_from_one_state() {
    ...
    stage.update(Some(&slide), None, &running(1.0));
...
fn stage_display_state_is_bounded_over_many_updates() {
    ...
    for i in 0..1000u32 {
        sd.update(Some(&Slide::title(format!("Line {i}"))), None, &running(1.0));
    }
