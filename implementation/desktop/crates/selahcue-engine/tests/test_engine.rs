...
fn command_event_sequence() {
    ...
    assert_eq!(
        e.apply(EngineCommand::SetScene { frame: solid(8, Rgba::WHITE) }),
        EngineEvent::FrameRendered { seq: 1 }
    );
    assert_eq!(
        e.apply(EngineCommand::InjectFault { fault: Fault::GpuDeviceLost }),
        EngineEvent::OutputHeld { fault: Fault::GpuDeviceLost }
    );
    assert_eq!(
        e.apply(EngineCommand::SetScene { frame: solid(8, Rgba::rgb(1, 2, 3)) }),
...
fn blackout_output_is_all_black() {
    ...
    e.apply(EngineCommand::SetScene { frame: solid(4, Rgba::WHITE) });
    e.apply(EngineCommand::Blackout { on: true });
    for y in 0..4 {
        for x in 0..4 {
...
fn output_never_blanks_across_a_fault() {
    let mut e = Engine::new(4, 4);
    e.apply(EngineCommand::SetScene { frame: solid(4, Rgba::WHITE) });
    e.apply(EngineCommand::InjectFault { fault: Fault::DiskFull });
...
fn ipc_commands_and_events_round_trip() {
    ...
    let commands = vec![
        EngineCommand::SetScene { frame: solid(2, Rgba::rgb(9, 9, 9)) },
        EngineCommand::Blackout { on: false },
        EngineCommand::Clear,
        EngineCommand::InjectFault { fault: Fault::IpcStall },
    ...
    let events = vec![
        ...
        EngineEvent::Recovered { seq: 2 },
        EngineEvent::Captured { seq: 2 },
        EngineEvent::Rejected { reason: "bad".into() },
    ];
    for ev in events {
...
fn oversized_frame_is_rejected_and_output_is_held() {
    ...
    e.apply(EngineCommand::SetScene { frame: solid(16, Rgba::WHITE) });
...
fn engine_state_is_bounded_over_many_commands() {
    ...
    for i in 0..5000u32 {
        let c = (i & 0xff) as u8;
        e.apply(EngineCommand::SetScene { frame: solid(32, Rgba::rgb(c, c, c)) });
    }
    assert_eq!(
        e.output().byte_len(),
        expected_bytes,
