...
**A future AI (or other off-by-default) feature that fails to reach `make launch`/`make operator` fails a build, not a checklist.** `scripts/check_launch_reachability.py` asserts `make -n launch`/`make -n operator`'s resolved `--features` list is a superset of the Cargo features tagged `# LAUNCH_REACHABILITY: REQUIRED` in `selahcue-operator/Cargo.toml`'s own `[features]` comments (currently `dev-keys`, `openai-notes`, `cloud-stt`) — wired into `make ci` and CI. The registry is no longer a hand-maintained list in the script (that was the shape of the second time this bug happened — 86akby7th's `cloud-stt` shipped invisible because the script predated it and nobody edited a separate file to register it, 86akd10dq): every non-`default` feature in that `[features]` table must carry a `REQUIRED`/`AUTO`/`OPT-IN` tag in its own comment block, and the script hard-fails if one is missing or unrecognised — a new feature cannot land without recording the reachability decision, in the same file and the same review a reviewer is already looking at. `.github/pull_request_template.md`'s "Feature-flag reachability" checklist complements this as a human prompt; it is not the control, since an unchecked box blocks nothing on its own.
