...
**A future AI (or other off-by-default) feature that fails to reach `make launch`/`make operator` fails a build, not a checklist.** `scripts/check_launch_reachability.py` asserts `make -n launch`/`make -n operator`'s resolved `--features` list is a superset of the Cargo features the product has decided must be default-reachable (currently `dev-keys`, `openai-notes`) — wired into `make ci` and CI. `.github/pull_request_template.md`'s "Feature-flag reachability" checklist complements this; it is not the control, since an unchecked box blocks nothing on its own.
